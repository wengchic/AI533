...
def evaluate_policy_discounted(env, policy, discount_factor, trials = 1000):
    ...
    for _ in range(trials):
        ...
        while not done:
            ...
            observation, reward, done, info = env.step(policy[observation])
            total_reward += pow(discount_factor, count) * reward
            count += 1
...
def VI_worker_v2(VI_server, data, start_state, end_state):
        ...
        # bellman backup
        max_v_arr = [0] * (end_state-start_state)
        max_a_arr = [0] * (end_state-start_state)        
        for update_state in range(start_state, end_state):
            ...
            for action in range(A):
                ...
                for state_next, prob in env.GetSuccessors(update_state, action):
                    # T(s,a,s′)V(s′)
                    sum_v += prob * V[state_next]

                # V(s)=maxR(s,a)+βT(s,a,s′)V(s′)
                v_a = env.GetReward(update_state, action) + beta * sum_v 

                if v_a > max_v:
                    max_a_arr[update_state-start_state] = action
                    max_v_arr[update_state-start_state] = v_a
                    max_v = v_a
        
        VI_server.update.remote(start_state, end_state, max_v_arr, max_a_arr)
...
def fast_value_iteration(env, beta = 0.999, epsilon = 0.01, workers_num = 4):
    ...
    #INSERT YOUR CODE HERE
    start = 0
    dis_size = [S // workers_num] * workers_num
    dis_size[-1] += 1
    error = float('inf')
    ...
    while error > epsilon:
        ...
        for i in range(workers_num):
            w_id = VI_worker_v2.remote(VI_server, data_id, start, start+dis_size[i])
